...
[Oh Shit, Git!?!](https://ohshitgit.com/)와 Git을 사용하면서 참고 할 만한 git 커맨드 치트 시트를 작성한 문서 </br>
git을 사용할 때 window 사용자라면 git bash를 기본 터미널로 사용하는 것을 강력 추천함.
mac os는 iterm2, linux는 bash가 기본 터미널로 사용하기 때문에 문제가 없음.
...
이 명령어를 실행하면 전체 커밋 로그가 열림.

2. **커밋 목록에서 합치고자 하는 커밋들 찾기**

텍스트 에디터에서 합치고자 하는 커밋들을 찾습니다. 가장 오래된 커밋부터 시작해서 `pick`을 `squash`(또는 `s`)로 변경합니다. 가장 최신 커밋의 `pick`은 그대로 두기

3. **커밋 메시지 수정하기**

파일을 저장하고 텍스트 에디터를 종료하면 다른 텍스트 에디터가 열림. 여기에서 새로운 커밋 메시지를 작성하기

4. **rebase 완료하기**

새로운 커밋 메시지를 작성하고 텍스트 에디터를 종료하면 rebase가 완료됨. 이제 `git log`를 실행하면 선택한 이전 커밋들이 하나의 커밋으로 합쳐진 것을 확인할 수 있음.
...
1. 첫번째 커밋 해시 찾기</br>
window 사용자분이시라면 git bash를 사용해야만 아래 명령어를 사용할 수 있음
